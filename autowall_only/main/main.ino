...
// --- Autonomous state machine settings ---
const float TARGET_CM         = 32.0; // desired distance from wall (sensor 1)
const float DEAD_BAND         = 3.0;  // no turning if |error| <= this
const float MAX_ERR           = 30.0; // clamp error used for scaling
const int   MIN_TURN_CYCLES   = 1;    // shortest turn when error is small
const int   MAX_TURN_CYCLES   = 2;    // longest turn when error is large

const int FORWARD_COOLDOWN    = 10;    // Y: forward cycles required before next turn

enum AutoState { FORWARDING, TURNING_LEFT, TURNING_RIGHT };
AutoState auto_state = FORWARDING;

int turn_cycles_left = 0;
int forward_cooldown_left = 3;

// Drive helpers (fix signatures)
inline void goForward(int sp){ drive(HIGH, 0, HIGH, 0, sp, sp); }
inline void turnLeft (int sp){ drive(HIGH, 0, HIGH, 0, 0,  sp); }  // left wheel stops, right forward
inline void turnRight(int sp){ drive(HIGH, 0, HIGH, 0, sp, 0 ); }  // left forward, right stops
inline void stopDrive(){ drive(0,0,0,0,0,0); }

// Map |error| to a turn length in cycles
...
void loop() {
  ...
  float error = cm1 - TARGET_CM;   // >0: too far from wall (turn LEFT to hug), <0: too close (turn RIGHT)
    ...
    case FORWARDING: {
      if (forward_cooldown_left > 0) {
        ...
        if (desired_turn_cycles > 0) {
          if (error > 0) {
            auto_state = TURNING_LEFT;
          } else {
            auto_state = TURNING_RIGHT;
          }
